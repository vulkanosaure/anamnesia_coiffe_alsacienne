...
package data2.display.scrollbar {
	...
	public dynamic class Scrollbar extends LayoutSprite {
		...
		private function onMouseDownTarget(e:MouseEvent):void 
		{
			//trace("Scrollbar.onMouseDownTarget");
			if (_lock) return;
		...
		private function onMouseUpTarget(e:MouseEvent):void 
		{
			//trace("Scrollbar.onMouseUpTarget");
			if (_lock) return;
		...
		private function onMouseMoveTarget(e:MouseEvent):void 
		{
			if (_lock) return;
			if (_touchMouseDown) {
				
				var _touchDelta:Number = this.mouseY - _touchPosition.y;
				//trace("onMouseMoveTarget, _touchDelta : " + _touchDelta);
